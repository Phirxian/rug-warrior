...
/*
 * Get the number of clicks and reset to 0
 */
int get_left_clicks()
{
    /* Same as above for right Shaft encoder */
    int clks = peek(0x1026);    /* PACNT is only an 8-bit register -- be sure to */
    poke(0x1026,0);             /* call it often enough so that it dosen't overflow */
    return clks;
}
